...
var tcd_strHelpText = "此脚本创建所选合成层次结构的完整副本，包括子合成.\n\
如果一个合成被多次使用，则该合成仅复制一次，并且所有剩余的引用都指向第一个复制.\n\
如果在项目面板中将合成排列在特殊的文件夹层次结构中，则会为复制的合成保留或复制该文件夹层次结构(取决于用户首选项.\n\
此版本通过更好地控制复制过程，为您节省了更多时间.\n\
新项目命名\
- 您可以为重复项目的名称添加前缀或后缀.\
\t这包括合成、文件夹和素材.\
- 您可以搜索和替换重复项目名称中的文本.\
...
\t使用不同的艺术/视频.\
\t注意：这实际上不会复制硬盘上的项目,\
\t只是他们在您的项目中的引用。 这不会复制固体,\
\t调整层或空对象.\n\
- 复制实体将复制合成中使用的实体.\n\
- 选择重复项将在复制后选择新的顶级重复项.\n\
您还可以指定要制作的份数.\
注意：每个副本名称中的最后一个数字将自动增加。 如果没有数字，将添加一个.\n\
此版本的脚本需要After Effects CS3或更高版本。 通过将脚本放置在Scripts文件夹的ScriptUI Panels子文件夹中，然后从“窗口”菜单中选择此脚本，可以将其用作可停靠面板。.";
var previousComps = [];
var previousFolders = [];
var previousFootage = [];
var tcd_prefsToSave = [];
...
function tcd_buildUI(thisObj) {
    ...
    if (myPal != null) {
        ...
        "group { \
            ...
            newNamesGrp: Panel { \
                ...
                replGrp: Group { \
                    ...
                    replReplLbl: StaticText {text:'更换', alignment:['left','left']}, \
                ...
                incGrp: Group { \
                    ...
                    incDrp: DropDownList {}, \
                    incLbl2: StaticText {text:' 名称中的数字', alignment:['fill','left']}, \
                }, \
                colGrp: Group { \
                    orientation: 'row', \
                    alignment: ['fill','top'], \
            ...
            optionsGrp: Panel { \
                ...
                grpFldGrp: Group { \
                    ...
                    grpFldLbl: StaticText {text:'将项目分组到文件夹中', alignment:['left','left']}, \
                ...
                dupFtgGrp: Group { \
                    ...
                    dupFtgLbl: StaticText {text:'素材重复 (慢)', alignment:['fill','left']}, \
                } \
                dupSldGrp: Group { \
                    orientation: 'row', \
                    alignment: ['fill','top'], \
                    dupSldChk: Checkbox {text:''}, \
                    dupSldLbl: StaticText {text:'重复实体', alignment:['fill','left']}, \
                } \
                selDupGrp: Group { \
                    orientation: 'row', \
                    alignment: ['fill','top'], \
                    selDupChk: Checkbox {text:''}, \
                    selDupLbl: StaticText {text:'选择重复项', alignment:['fill','left']}, \
                } \
            }, \
            toolsGrp: Panel { \
                alignment: ['fill','top'], \
                ...
                collectGrp: Group { \
                    orientation: 'row', \
                    collectBtn: Button {text:'收集依赖', alignment:['right','top']} \
                } \
            }, \
            btnGrp: Group { \
                orientation: 'row', \
                ...
                copyLbl: StaticText {text:'份数', alignment:['right','center']}, \
                copyTxt: EditText {text:'1', alignment:['right','top'], preferredSize:[30,20]}, \
                dupSelBtn: Button {text:' 重复选择 ', alignment:['right','top']}, \
            } \
        }";
        ...
        // Exclude Drop Down
        var incExcOptionsB = ["前缀","后缀","匹配的正则表达式"];
        ...
        myPal.grp.btnGrp.dupSelBtn.onClick = function() {
            ...
            if (myPal.grp.newNamesGrp.preSufGrp.preSufChk.value) {
                if (myPal.grp.newNamesGrp.preSufGrp.preSufTxt.text === "") {
                    errors.push("没有为提供值 " + myPal.grp.newNamesGrp.preSufGrp.preSufDrp.selection.text);
                }
            }
            ...
            if (myPal.grp.optionsGrp.incExcGrp.incExcChk.value) {
                if (myPal.grp.optionsGrp.incExcGrp.incExcTxt.text == "") {
                    errors.push("没有为提供值 " + myPal.grp.optionsGrp.incExcGrp.incExcLbl.text + " " + myPal.grp.optionsGrp.incExcGrp.incExcDrp.selection.text);
                }
            ...
            if (myPal.grp.optionsGrp.grpFldGrp.grpFldChk.value) {
                if (myPal.grp.optionsGrp.grpFldGrp.grpFldTxt.text == "") {
                    errors.push("没有为提供值 " + myPal.grp.optionsGrp.grpFldGrp.grpFldLbl.text);
                }
            }
        ...
        myPal.grp.toolsGrp.collectGrp.collectBtn.onClick = function() {
            ...
            if (myPal.grp.optionsGrp.grpFldGrp.grpFldChk.value) {
                ...
                errors.push("若要收集依赖项，必须启用“将项目分组到文件夹”'")
            }
            ...
            for (var i=0; i<selItems.length; i++) {
                if (!(selItems[i] instanceof CompItem)) {
                    errors.push("所选内容包含组件以外的项目，请仅选择要为其收集依赖项的顶级组件.");
                    break;
               }
            }
...
helpWindow.prototype.run = function() {
    ...
    win.btnPanel.aesBtn = win.btnPanel.add("button", [310, 550,450, 580], "水墨轩汉化");

    win.btnPanel.aesBtn.onClick = function() {
        openURL("https://www.smxae.com");
    };
